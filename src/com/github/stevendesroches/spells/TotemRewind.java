...
public class TotemRewind extends TargetedSpell implements TargetedLocationSpell {
    ...
    private Map<UUID, Double> playerList;
    private Map<UUID, Long> playerCD;
    ...
    public TotemRewind(MagicConfig config, String spellName) {
    ...
    public class Totem {
        ...
        private boolean activate() {
            ...
            Collection<Entity> collection = armorStand.getNearbyEntities(5.0, 5.0, 5.0);
            for (Entity entity : collection) {
                if (entity instanceof Player) {
                    if (!playerList.containsKey(entity.getUniqueId())) {
                        playerList.put(entity.getUniqueId(), ((Player) entity).getHealth());
                        playerCD.put(entity.getUniqueId(), (System.currentTimeMillis() + 3000));
                    } else {
                        if (playerCD.get(entity.getUniqueId()) <= System.currentTimeMillis()) {
                            ((Player) entity).setHealth(playerList.get(entity.getUniqueId()));
                            playerCD.put(entity.getUniqueId(), (System.currentTimeMillis() + 300000));
                        }
                    }
                }
            }
        ...
        public void stop() {
            ...
            armorStand.remove();
            for (Map.Entry mapElement : playerCD.entrySet()) {
                if ((long) mapElement.getValue() <= System.currentTimeMillis()) {
                    playerCD.put((UUID) mapElement.getKey(), (System.currentTimeMillis() + 300000));
                }
            }
            playerCD.clear();
            playerList.clear();
